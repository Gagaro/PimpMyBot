# https://github.com/Outernet-Project/bottle-utils/blob/master/bottle_utils/csrf.py
import functools
import hashlib
import os
...
def get_conf():
    ...
    csrf_secret = conf.get('csrf.secret', 'SECRET')  # FIXME Default secret
...
def csrf_protect(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if six.text_type(form_token) != token.decode('utf8'):
...
def csrf_tag():
    ...
    try:
        ...
        pass
    return Markup('<input type="hidden" name="{0}" value="{1}" />'.format(token_name, token))
